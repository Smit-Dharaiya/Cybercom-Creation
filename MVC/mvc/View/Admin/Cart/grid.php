                        ...
                        <tr>
                            <?php if ($columns) : ?>
                                <?php foreach ($columns as $key => $column) : ?>
                                    ...
                                    <?php else : ?>
                                        <?php if ($key == 'quantity') : ?>
                                            <td><input type="number" class="form-control" min=1 name="<?php echo $this->getFieldValue($row, 'id'); ?>[quantity]" value="<?php echo $this->getFieldValue($row, $column['field']); ?>" /></td>
                                        <?php else : ?>
                                            <?php if ($key == 'discount') : ?>
                                                <td><?php echo ($this->getFieldValue($row, 'discount')); ?></td>
                                            <?php else : ?>
                                                <?php if ($key == 'changedPrice') : ?>
                                                    <?php //echo (($this->getFieldValue($row, 'quantity')) * ($this->getFieldValue($row, 'price')) - (($this->getFieldValue($row, 'discount')) * ($this->getFieldValue($row, 'quantity'))));
                                                    ?>
                                                    <td><input type="number" class="form-control" name="<?php echo $this->getFieldValue($row, 'id'); ?>[changedPrice]" value="<?php echo $this->getFieldValue($row, 'changedPrice'); ?>" /></td>
                                                <?php else : ?>
